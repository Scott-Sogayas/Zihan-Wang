...
class AIFI_CloFormer(nn.Module):
    ...
    1. 增加参数自动匹配逻辑
    2. 修正 cm (FFN维度) 的默认比例
    3. 将 CloFormer-S 的配置集成进来
    """
    def __init__(self, c1, cm=None, num_heads=None, dropout=0.0, act=nn.GELU()):
        super().__init__()
        if num_heads is None:
            if c1 <= 64:    # Stage 1 风格
                num_heads, group_split, kernel_sizes, window_size = 4, [3, 1], [3], 8
            elif c1 <= 128: # Stage 2 风格
                num_heads, group_split, kernel_sizes, window_size = 8, [4, 4], [5], 4
            elif c1 <= 256: # Stage 3 风格
                num_heads, group_split, kernel_sizes, window_size = 14, [7, 7], [7], 2
            else:           # Stage 4 风格
                num_heads, group_split, kernel_sizes, window_size = 28, [7, 21], [9], 1
        else:
            # 如果你手动传了 num_heads，这里需要补全其他默认值（可选）
            group_split = [num_heads // 2, num_heads // 2]
            kernel_sizes = [7]
            window_size = 2

        # 修正 cm：Transformer 标准做法是 c1 的 4 倍，原代码固定 2048 太大或太小都不好
        if cm is None:
            cm = c1 * 4

        # 实例化真正的 CloFormer 核心
        self.Attention = CloFormerAttnConv(
            dim=c1, 
            num_heads=num_heads, 
    ...
    def forward(self, src):
        # 建议使用更能保持梯度的结构（先做注意力，再做残差，再做 Norm）
        # 这里维持你原来的 forward 逻辑以减少结构改动
