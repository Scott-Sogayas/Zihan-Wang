...
import numpy as np
import torch
from .MoCAttention import MoCAttention, makeDivisible

__all__ = ['efficientnet_v2','MBConv','sev2']
        ...
        if c.expand_ratio == 1:
            ...
            block.append(('depth_wise',
                          ConvBNAct(inter_channel, inter_channel, c.kernel, c.stride, inter_channel, c.norm_layer,
                                    c.act)))
            block.append(('se', MoCAttention(InChannels=inter_channel, SqueezeFactor=4 * c.expand_ratio)))##################################
...
def efficientnet_v2(model_name='efficientnet_v2_l', pretrained=False, nclass=0, dropout=0.1, stochastic_depth=0.2,
                    **kwargs):
...
if __name__ == "__main__":
    ...
    # Model
    model = efficientnet_v2('efficientnet_v2_l')
