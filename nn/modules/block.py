...
class BiFPN(nn.Module):
    def __init__(self, length, out_channels=256):
        super().__init__()
        self.length = length  # 输入特征图的数量
        self.out_channels = out_channels  # 统一输出通道数
        ...
        self._conv_initialized = False  # 标记卷积层是否已初始化

    def forward(self, x):

        # 动态初始化通道适配卷积层（解决负维度问题）
        if not self._conv_initialized:
            # 遍历每个输入特征图，根据实际通道数创建1x1卷积
            for i in range(self.length):
                in_ch = x[i].shape[1]  # 获取当前特征图的实际输入通道数
                conv = nn.Conv2d(
                    in_channels=in_ch,
                    ...
                    padding=0,
                    bias=False
                ).to(x[0].device)  # 确保卷积层和输入在同一设备（GPU/CPU）
                # 初始化卷积权重
                nn.init.normal_(conv.weight, mean=0.0, std=0.01)
                self.channel_adapt.append(conv)
            self._conv_initialized = True  # 标记为已初始化

        adapted_features = []
        for i, feat in enumerate(x):
            adapted_feat = self.channel_adapt[i](feat)
...
class RepC3(nn.Module): 
    # LazyMan 建议：增加 shortcut 参数
    def __init__(self, c1: int, c2: int, n: int = 3, e: float = 1.0, shortcut: bool = True): 
        super().__init__() 
        c_ = int(c2 * e) 
        self.cv1 = Conv(c1, c_, 1, 1) 
        self.cv2 = Conv(c1, c_, 1, 1) 
        ...
        # 新增项目：当输入输出通道一致且开启 shortcut 时，使用残差
        self.add_shortcut = shortcut and c1 == c2 

    def forward(self, x: torch.Tensor) -> torch.Tensor: 
        # 原有的分支融合逻辑
        out = self.cv3(self.m(self.cv1(x)) + self.cv2(x))
        
        # LazyMan 的残差改造：如果开启了 shortcut，则加上原始输入 x
